...
import { SiSanity, SiTypescript, SiMongodb, SiNodedotjs } from "react-icons/si";
...
type StackItem = { label: string; Icon: IconType; highlight?: boolean };
...
type Project = {
  ...
  frontend: StackItem[];
  backend: StackItem[];
  image: typeof cvaldavImage;
  imageAlt: string;
};

/** Primele două tehnologii din frontend sunt evidențiate (ca în mockupul HTML). */
...
function ProjectSlide({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <Card className="group overflow-hidden rounded-2xl border border-[var(--card-border)] bg-[var(--card-bg)] p-8 shadow-none transition-colors duration-300 hover:border-[rgba(74,144,226,0.25)] md:p-10">
      ...
      <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-14">
        ...
        <div className="relative mx-auto aspect-[16/10] w-full max-w-xl overflow-hidden rounded-[10px] border border-white/[0.07] bg-white/[0.03] lg:mx-0 lg:max-w-none">
          <span
            className="pointer-events-none absolute -right-1 top-0 select-none font-display text-[clamp(4rem,14vw,10rem)] font-bold leading-none text-white/[0.06]"
            aria-hidden
          >
            {n}
          </span>
...
export const ProjectsSection = () => {
  ...
  return (
    <section
      id="projects"
      className="bg-[var(--bg-section)] pb-20 pt-12 md:pb-24 md:pt-16"
    >
            ...
            <CarouselContent className="-ml-0">
              {projects.map((project, projectIndex) => (
                <CarouselItem key={project.id} className="pl-0">
                  <ProjectSlide project={project} index={projectIndex} />
                </CarouselItem>
              ))}
            </CarouselContent>
